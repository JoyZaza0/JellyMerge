...
half3 RimDirLightingHalf(half rimMin, half rimMax, half4 rimColor, half3 viewDirection, half3 worldNormal, half3 worldPos) {
	half3 viewDir = normalize(viewDirection);
	half rimTemp = smoothstep(rimMin, rimMax, 1 - saturate(dot(viewDir, worldNormal)));

	half3 lightDir = LightDirectionHalf(worldPos);
